...
if __name__ == "__main__":
    ...
    # 1. sparksessionを作成
    spark = SparkSession.builder.appName("KafkaToParquetPartitioned").getOrCreate()
    ...
    # 7. パース結果をParquet形式のファイルに出力
    query = withMonth.writeStream\
                        .partitionBy("id", "month")\
                        .outputMode("append")\
                        .format("parquet")\
                        .option("path", "/opt/spark-book/chapter06/data/parquetPartitioned")\
                        .option("checkpointLocation", "/opt/spark-book/chapter06/state/KafkaToParquetPartitioned")\
                        .start()
